...
ScaleParameters ScaleParameters::makeScaled(int Warehouses,
  ...
  return ScaleParameters(items, Warehouses, districts, customers, newOrders);
    ...
    void RandomHelper::generateDistrict(int did, int wid, int nextOId, District& out) {
        out.dId = did;
        out.dWId = wid;
    ...
    void RandomHelper::generateCustomer(int cwid, int cdid, int cid, bool badCredit, Customer& out) {
    ...
    void RandomHelper::generateOrder(int owid, int odid, int oid, int ocid, int oilcnt, bool newOrder, Order& out) {
        out.oEntryD = chrono::system_clock::now();
        out.oCarrierId = newOrder ? NULL_CARRIER_ID : number(MIN_CARRIER_ID, MAX_CARRIER_ID);
    ...
    void RandomHelper::generateOrderLine(const ScaleParameters& params, int olwid, int oldid, int oloid, int olnumber, int maxitems, bool newOrder, OrderLine& out) {
        out.olIId = number(1, maxitems);
        out.olSupplyWId = olwid;
        out.olDeliveryD = newOrder ? chrono::system_clock::time_point(0s) : chrono::system_clock::now();
        out.olQuantity = INITIAL_QUANTITY;

        bool remote = number(1,100) == 1;
        if(params.warehouses > 0 && remote) {
            out.olSupplyWId = numberExcluding(params.startingWarehouse, params.endingWarehouse, olwid);
        }

        out.olAmount = fixedPoint(MONEY_DECIMALS, MAX_PRICE * MAX_OL_QUANTITY, MIN_AMOUNT);
    ...
    void RandomHelper::generateStock(int swid, int siid, bool original, Stock& out) {
        out.sIId = siid;
        out.sWId = swid;
    ...
    void RandomHelper::generateHistory(int hcwid, int hcdid, int hcid, History& out) {
        out.hCWId = hcwid;
        out.hCDId = hcdid;
        out.hCId = hcid;
        out.hWId = hcwid;
